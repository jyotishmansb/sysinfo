...
- **The File Actions (CRUD)**:
  ...
  - **`updateFile` (Update)**: When you want to change what's inside a file, this function deletes the old text and replaces it entirely with the new text you provide. Just like reading, it double-checks that the file actually exists before trying to update it, preventing you from accidentally creating files when you meant to edit them.
